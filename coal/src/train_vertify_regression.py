...
def train(train_set,view_data_path):
    ...
    # 回归评价标准MSE
    MSE = mean_squared_error(test_set_maxValue, maxValue_predict)
    print("根据浓度值回归后MSE：", MSE)
    ...
    plt.plot(data_date, test_set_maxValue_limit, 'go-', label='真实浓度值', linestyle="-")
    plt.plot(data_date, maxValue_predict_limit, 'ro-', label='预测浓度值', linestyle="--")
    plt.axhline(0.4, color="red", linewidth=1.0, linestyle='--', label='瓦斯风险等级2')
    plt.axhline(0.7, color="black", linewidth=1.0, linestyle='--', label='瓦斯风险等级3')
    plt.axhline(1.0, color="blue", linewidth=1.0, linestyle='--', label='瓦斯风险等级4')
    # 配置横坐标
    ...
    fig_title = "未采用DVC算法预测结果"
    plt.title(fig_title, fontdict={'weight': 'normal', 'size': 25})
    plt.legend(loc="upper right")
    ax.set_xlabel("日期", fontdict={'weight': 'normal', 'size': 25})
    ax.set_ylabel("预测浓度", fontdict={'weight': 'normal', 'size': 25})
    fig_save_path = str("../data/") + str(test_id) + str(model) + str("--all") + str(".png")
    ...
    maxValue_predict_v2_limit = maxValue_predict_v2[300:1000]
    plt.plot(data_date, test_set_maxValue_limit, 'go-', label='true value', linestyle="-")
    plt.plot(data_date, maxValue_predict_v2_limit, 'ro-', label='predict value', linestyle="--")
    plt.axhline(0.4, color="red", linewidth=1.0, linestyle='--', label='grade_2')
    plt.axhline(0.7, color="black", linewidth=1.0, linestyle='--', label='grade_3')
    plt.axhline(1.0, color="blue", linewidth=1.0, linestyle='--', label='grade_4')
    # 配置横坐标
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y/%m/%d'))
    # ax.xaxis.set_minor_locator(mticker.MultipleLocator(10))
    mticker.Locator.MAXTICKS = 2800
    ...
    model = "--LinearRegression"
    fig_title = str("pointID:") + str(test_id) + str(model) + str("--v2") + str("--MSE:") + str(MSE)
    plt.title(fig_title, fontdict={'weight': 'normal', 'size': 25})
    plt.legend(loc="upper right")
    ax.set_xlabel("日期", fontdict={'weight': 'normal', 'size': 25})
    ax.set_ylabel("预测浓度", fontdict={'weight': 'normal', 'size': 25})
    fig_save_path = str("../data/") + str(test_id) + str(model) + str("--v2") + str(".png")
    plt.savefig(fig_save_path)
    # plt.close()
    print("--------------------------------------------------------------")
    ...
    # # # 作图展示
    # plt.figure(figsize=(1000, 10), dpi=99)
    fig = plt.figure(figsize=(20, 10), dpi=90)
    ax = fig.add_subplot(1, 1, 1)
    # test_set_maxValue_limit = test_set_maxValue[0:1400]
    vote_predict_mean_lianxv_limit = vote_predict_mean_lianxv[300:1000]
    plt.plot(data_date, test_set_maxValue_limit, 'go-', label='真实浓度值', linestyle="-")
    plt.plot(data_date, vote_predict_mean_lianxv_limit, 'ro-', label='预测浓度值', linestyle="--")
    plt.axhline(0.4, color="red", linewidth=1.0, linestyle='--', label='瓦斯风险等级2')
    plt.axhline(0.7, color="black", linewidth=1.0, linestyle='--', label='瓦斯风险等级3')
    plt.axhline(1.0, color="blue", linewidth=1.0, linestyle='--', label='瓦斯风险等级4')
    # 配置横坐标
    ...
    model = "--LinearRegression"
    fig_title = "采用DVC算法预测结果"
    plt.title(fig_title, fontdict={'weight': 'normal', 'size': 25})
    plt.legend(loc="upper right")
    ax.set_xlabel("日期", fontdict={'weight': 'normal', 'size': 25})
    ax.set_ylabel("预测浓度", fontdict={'weight': 'normal', 'size': 25})
    # plt.show()
    fig_save_path = str("../data/") + str(test_id) + str(model) + str("--vote_predict_mean") + str(".png")
    plt.savefig(fig_save_path)
    plt.close()

    return accuracy,score_vote_mean,score_vote_grade
...
if __name__ == '__main__':
    ...
    for view_data_path in view_data_path_list:
        ...
        for i in range(num_iter):
            allData_acc, score_vote_mean, score_vote_grade = train(train_set, view_data_path)
