...
def try_different_cluster_model(model,data,num_cluster):
    ...
    # print(cluster_result_groupby)
    # 将视图存为json文件
    jsObj = json.dumps(cluster_result_groupby)
    view_save_path = str("../data/featureClusterResult/")+str(namestr(model, globals())[0])+str("--")+str(num_cluster)+\
...
if __name__ == '__main__':
    df = pd.read_csv("../data/experiment_data/MI_info_withLabel_discretion.csv", header=None)
    ...
    try_different_cluster_model(kmeans_model, df_array, num_cluster)

    # 高斯混合聚类
    gmmModel = GaussianMixture(n_components=num_cluster, covariance_type='diag', random_state=0)
    try_different_cluster_model(gmmModel, df_array, num_cluster)
